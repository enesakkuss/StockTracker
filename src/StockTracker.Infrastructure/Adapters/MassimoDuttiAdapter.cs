...
/// <summary>
/// Universal store adapter for Massimo Dutti (massimodutti.com).
/// Part of the Inditex group.
/// </summary>
public class MassimoDuttiAdapter : IStoreAdapter, IInspectableAdapter
{
    private readonly IBrowserService _browserService;
    private readonly ILogger<MassimoDuttiAdapter> _logger;

    private static readonly Regex MassimoDuttiUrlPattern =
        new(@"https?://([a-zA-Z0-9_.-]+\.)?massimodutti\.com(/|$|\?)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);

    public string StoreName => "Massimo Dutti";
    public string AdapterKey => "massimodutti";
    public IReadOnlyList<string> SupportedDomains { get; } = new[] { "massimodutti.com" };

    public MassimoDuttiAdapter(IBrowserService browserService, ILogger<MassimoDuttiAdapter> logger)
    {
    ...
    public bool CanHandle(string url)
    {
        ...
        return MassimoDuttiUrlPattern.IsMatch(url.Trim());
    }
    ...
    public async Task<ProductInspectResponse> InspectAsync(string url, CancellationToken cancellationToken = default)
    {
        if (!CanHandle(url))
        {
            throw new NotSupportedException($"MassimoDuttiAdapter bu URL'yi desteklemiyor: {url}");
        }

        _logger.LogInformation("Inspecting Massimo Dutti product: {Url}", url);
        ...
        try
        {
            ...
            try
            {
                await page.WaitForSelectorAsync(
                    "script[type='application/ld+json'], button[data-testid*='size'], button.size-item, .size-selector, h1",
            ...
            {
                _logger.LogInformation("Massimo Dutti: Parsed via JSON-LD: {Name} ({Count} variants)", jsonLdResult.Name, jsonLdResult.Variants.Count);
                return jsonLdResult;
            }
            ...
            if (interceptedJson is not null)
            {
                ...
                {
                    _logger.LogInformation("Massimo Dutti: Parsed via Intercepted API: {Name} ({Count} variants)", apiResult.Name, apiResult.Variants.Count);
                    return apiResult;
                }
            }
            ...
            {
                _logger.LogInformation("Massimo Dutti: Parsed via Embedded State: {Name} ({Count} variants)", stateResult.Name, stateResult.Variants.Count);
                return stateResult;
            }
            ...
            {
                _logger.LogInformation("Massimo Dutti: Parsed via DOM: {Name} ({Count} variants)", domResult.Name, domResult.Variants.Count);
                return domResult;
            }
    ...
    private ProductInspectResponse? ParseJsonLdElement(JsonElement root, string url)
    {
        ...
        var name = root.TryGetProperty("name", out var nameProp) ? nameProp.GetString() ?? "Massimo Dutti Ürün" : "Massimo Dutti Ürün";
    ...
    public ProductInspectResponse? TryParseInterceptedJson(string json, string url)
    {
        try
        {
            ...
            var name = productElem.TryGetProperty("name", out var np) ? np.GetString() ?? "Massimo Dutti Ürün" : "Massimo Dutti Ürün";
    ...
    private async Task<ProductInspectResponse?> TryParseDomAsync(IPage page, string url)
    {
        ...
        var imageUrl = await page.EvaluateAsync<string?>(@"() => {
            const img = document.querySelector('.product-media img, .media-image img, img.product-image, [data-testid=""product-image""] img');
        ...
        var variantsJson = await page.EvaluateAsync<string>(@"() => {
            ...
            const sizeButtons = document.querySelectorAll(
                'button[data-testid*=""size""], button.size-item, button.size-list-item, .size-selector button, .sizes-list button, .product-size button, [aria-label*=""Size""], [aria-label*=""Beden""]'
            );
    ...
    {
        if (string.IsNullOrWhiteSpace(title)) return "Massimo Dutti Ürün";
        var clean = Regex.Replace(title, @"\s*[-|]\s*Massimo Dutti.*$", "", RegexOptions.IgnoreCase).Trim();
