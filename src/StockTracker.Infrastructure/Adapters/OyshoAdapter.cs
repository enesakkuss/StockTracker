...
/// <summary>
/// Universal store adapter for Oysho (oysho.com).
/// Part of the Inditex group.
/// </summary>
public class OyshoAdapter : IStoreAdapter, IInspectableAdapter
{
    private readonly IBrowserService _browserService;
    private readonly ILogger<OyshoAdapter> _logger;

    private static readonly Regex OyshoUrlPattern =
        new(@"https?://([a-zA-Z0-9_.-]+\.)?oysho\.com(/|$|\?)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);

    public string StoreName => "Oysho";
    public string AdapterKey => "oysho";
    public IReadOnlyList<string> SupportedDomains { get; } = new[] { "oysho.com" };

    public OyshoAdapter(IBrowserService browserService, ILogger<OyshoAdapter> logger)
    {
    ...
    public bool CanHandle(string url)
    {
        ...
        return OyshoUrlPattern.IsMatch(url.Trim());
    }
    ...
    public async Task<ProductInspectResponse> InspectAsync(string url, CancellationToken cancellationToken = default)
    {
        if (!CanHandle(url))
        {
            throw new NotSupportedException($"OyshoAdapter bu URL'yi desteklemiyor: {url}");
        }

        _logger.LogInformation("Inspecting Oysho product: {Url}", url);
        ...
        try
        {
            ...
            {
                _logger.LogInformation("Oysho: Parsed via JSON-LD: {Name} ({Count} variants)", jsonLdResult.Name, jsonLdResult.Variants.Count);
                return jsonLdResult;
            }
            ...
            if (interceptedJson is not null)
            {
                ...
                {
                    _logger.LogInformation("Oysho: Parsed via Intercepted API: {Name} ({Count} variants)", apiResult.Name, apiResult.Variants.Count);
                    return apiResult;
                }
            }
            ...
            {
                _logger.LogInformation("Oysho: Parsed via Embedded State: {Name} ({Count} variants)", stateResult.Name, stateResult.Variants.Count);
                return stateResult;
            }
            ...
            {
                _logger.LogInformation("Oysho: Parsed via DOM: {Name} ({Count} variants)", domResult.Name, domResult.Variants.Count);
                return domResult;
            }
    ...
    private ProductInspectResponse? ParseJsonLdElement(JsonElement root, string url)
    {
        ...
        var name = root.TryGetProperty("name", out var nameProp) ? nameProp.GetString() ?? "Oysho Ürün" : "Oysho Ürün";
    ...
    public ProductInspectResponse? TryParseInterceptedJson(string json, string url)
    {
        try
        {
            ...
            var name = productElem.TryGetProperty("name", out var np) ? np.GetString() ?? "Oysho Ürün" : "Oysho Ürün";
    ...
    {
        if (string.IsNullOrWhiteSpace(title)) return "Oysho Ürün";
        var clean = Regex.Replace(title, @"\s*[-|]\s*Oysho.*$", "", RegexOptions.IgnoreCase).Trim();
