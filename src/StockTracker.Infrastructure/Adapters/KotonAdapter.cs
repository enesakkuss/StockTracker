...
/// <summary>
/// Universal store adapter for Koton (koton.com).
/// </summary>
public class KotonAdapter : IStoreAdapter, IInspectableAdapter
{
    private readonly IBrowserService _browserService;
    private readonly ILogger<KotonAdapter> _logger;

    private static readonly Regex KotonUrlPattern =
        new(@"https?://([a-zA-Z0-9_.-]+\.)?koton\.com(/|$|\?)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);

    public string StoreName => "Koton";
    public string AdapterKey => "koton";
    public IReadOnlyList<string> SupportedDomains { get; } = new[] { "koton.com" };

    public KotonAdapter(IBrowserService browserService, ILogger<KotonAdapter> logger)
    {
    ...
    public bool CanHandle(string url)
    {
        ...
        return KotonUrlPattern.IsMatch(url.Trim());
    }
    ...
    public async Task<ProductInspectResponse> InspectAsync(string url, CancellationToken cancellationToken = default)
    {
        if (!CanHandle(url))
        {
            throw new NotSupportedException($"KotonAdapter bu URL'yi desteklemiyor: {url}");
        }

        _logger.LogInformation("Inspecting Koton product: {Url}", url);
        ...
        try
        {
            ...
            page.Response += async (_, response) =>
            {
                try
                {
                    ...
                    if ((rUrl.Contains("/api/") || rUrl.Contains("/product") || rUrl.Contains("/variant")) &&
                        response.Headers.TryGetValue("content-type", out var ct) && ct.Contains("json"))
                    {
                        var text = await response.TextAsync();
                        if (text.Contains("\"sizes\"") || text.Contains("\"variants\"") || text.Contains("\"stock\""))
            ...
            if (gotoResponse.Status is 403 or 429)
                throw new InvalidOperationException($"Koton sayfası erişimi reddetti (HTTP {gotoResponse.Status} - Bot Koruması/Rate Limit).");
            ...
            {
                throw new InvalidOperationException("Koton sayfası bot koruması nedeniyle erişimi engelledi.");
            }

            try
            {
                await page.WaitForSelectorAsync(
                    "script[type='application/ld+json'], .size-area, .product-size, button[data-size], h1",
            ...
            {
                _logger.LogInformation("Koton: Parsed via JSON-LD: {Name} ({Count} variants)", jsonLdResult.Name, jsonLdResult.Variants.Count);
                return jsonLdResult;
            }
            ...
            if (interceptedJson is not null)
            {
                ...
                {
                    _logger.LogInformation("Koton: Parsed via Intercepted API: {Name} ({Count} variants)", apiResult.Name, apiResult.Variants.Count);
                    return apiResult;
                }
            }
            ...
            {
                _logger.LogInformation("Koton: Parsed via Embedded State: {Name} ({Count} variants)", stateResult.Name, stateResult.Variants.Count);
                return stateResult;
            }
            ...
            {
                _logger.LogInformation("Koton: Parsed via DOM: {Name} ({Count} variants)", domResult.Name, domResult.Variants.Count);
                return domResult;
            }
    ...
    private ProductInspectResponse? ParseJsonLdElement(JsonElement root, string url)
    {
        ...
        var name = root.TryGetProperty("name", out var nameProp) ? nameProp.GetString() ?? "Koton Ürün" : "Koton Ürün";
    ...
    public ProductInspectResponse? TryParseInterceptedJson(string json, string url)
    {
        try
        {
            ...
            var name = root.TryGetProperty("name", out var np) ? np.GetString() ?? "Koton Ürün" : "Koton Ürün";

            var variants = new List<VariantAvailabilityDto>();

            if (root.TryGetProperty("variants", out var varsElem) && varsElem.ValueKind == JsonValueKind.Array)
            {
                foreach (var v in varsElem.EnumerateArray())
                {
                    var sName = v.TryGetProperty("size", out var sp) ? sp.GetString()
                              : v.TryGetProperty("name", out var nmp) ? nmp.GetString() : null;

                    var isAvail = v.TryGetProperty("inStock", out var isp) ? isp.GetBoolean()
                                : v.TryGetProperty("stock", out var stp) && stp.TryGetInt32(out var st) && st > 0;
    ...
    private async Task<ProductInspectResponse?> TryParseDomAsync(IPage page, string url)
    {
        var name = await page.EvaluateAsync<string>(@"() => {
            const h1 = document.querySelector('h1.product-name, h1, .product-title, [data-testid=""product-name""]');
        ...
        var imageUrl = await page.EvaluateAsync<string?>(@"() => {
            const img = document.querySelector('.product-images img, .product-detail-images img, img.main-image, img.product-image');
        ...
        var variantsJson = await page.EvaluateAsync<string>(@"() => {
            ...
            const sizeButtons = document.querySelectorAll(
                'button[data-size], .size-area button, .product-size button, .size-box, [data-testid*=""size""], [aria-label*=""Beden""]'
            );
    ...
    {
        if (string.IsNullOrWhiteSpace(title)) return "Koton Ürün";
        var clean = Regex.Replace(title, @"\s*[-|]\s*Koton.*$", "", RegexOptions.IgnoreCase).Trim();
