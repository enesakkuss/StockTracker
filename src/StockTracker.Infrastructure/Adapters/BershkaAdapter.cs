...
/// <summary>
/// Universal store adapter for Bershka (bershka.com).
/// Part of the Inditex group.
/// </summary>
public class BershkaAdapter : IStoreAdapter, IInspectableAdapter
{
    private readonly IBrowserService _browserService;
    private readonly ILogger<BershkaAdapter> _logger;

    private static readonly Regex BershkaUrlPattern =
        new(@"https?://([a-zA-Z0-9_.-]+\.)?bershka\.com(/|$|\?)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);

    public string StoreName => "Bershka";
    public string AdapterKey => "bershka";
    public IReadOnlyList<string> SupportedDomains { get; } = new[] { "bershka.com" };

    public BershkaAdapter(IBrowserService browserService, ILogger<BershkaAdapter> logger)
    {
    ...
    public bool CanHandle(string url)
    {
        ...
        return BershkaUrlPattern.IsMatch(url.Trim());
    }
    ...
    public async Task<ProductInspectResponse> InspectAsync(string url, CancellationToken cancellationToken = default)
    {
        if (!CanHandle(url))
        {
            throw new NotSupportedException($"BershkaAdapter bu URL'yi desteklemiyor: {url}");
        }

        _logger.LogInformation("Inspecting Bershka product: {Url}", url);
        ...
        try
        {
            ...
            try
            {
                await page.WaitForSelectorAsync(
                    "script[type='application/ld+json'], button[data-qa-anchor*='size'], button.size-list-item, .sizes-list, .ui-size-selector, h1",
            ...
            {
                _logger.LogInformation("Bershka: Parsed via JSON-LD: {Name} ({Count} variants)", jsonLdResult.Name, jsonLdResult.Variants.Count);
                return jsonLdResult;
            }
            ...
            if (interceptedJson is not null)
            {
                ...
                {
                    _logger.LogInformation("Bershka: Parsed via Intercepted API: {Name} ({Count} variants)", apiResult.Name, apiResult.Variants.Count);
                    return apiResult;
                }
            }
            ...
            {
                _logger.LogInformation("Bershka: Parsed via Embedded State: {Name} ({Count} variants)", stateResult.Name, stateResult.Variants.Count);
                return stateResult;
            }
            ...
            {
                _logger.LogInformation("Bershka: Parsed via DOM: {Name} ({Count} variants)", domResult.Name, domResult.Variants.Count);
                return domResult;
            }
    ...
    private ProductInspectResponse? ParseJsonLdElement(JsonElement root, string url)
    {
        ...
        var name = root.TryGetProperty("name", out var nameProp) ? nameProp.GetString() ?? "Bershka Ürün" : "Bershka Ürün";
    ...
    public ProductInspectResponse? TryParseInterceptedJson(string json, string url)
    {
        try
        {
            ...
            var name = productElem.TryGetProperty("name", out var np) ? np.GetString() ?? "Bershka Ürün" : "Bershka Ürün";
    ...
    {
        if (string.IsNullOrWhiteSpace(title)) return "Bershka Ürün";
        var clean = Regex.Replace(title, @"\s*[-|]\s*Bershka.*$", "", RegexOptions.IgnoreCase).Trim();
