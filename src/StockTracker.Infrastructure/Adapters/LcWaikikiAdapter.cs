...
/// <summary>
/// Universal store adapter for LC Waikiki (lcwaikiki.com).
/// </summary>
public class LcWaikikiAdapter : IStoreAdapter, IInspectableAdapter
{
    private readonly IBrowserService _browserService;
    private readonly ILogger<LcWaikikiAdapter> _logger;

    private static readonly Regex LcWaikikiUrlPattern =
        new(@"https?://([a-zA-Z0-9_.-]+\.)?lcwaikiki\.com(/|$|\?)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);

    public string StoreName => "LC Waikiki";
    public string AdapterKey => "lcwaikiki";
    public IReadOnlyList<string> SupportedDomains { get; } = new[] { "lcwaikiki.com" };

    public LcWaikikiAdapter(IBrowserService browserService, ILogger<LcWaikikiAdapter> logger)
    {
    ...
    public bool CanHandle(string url)
    {
        ...
        return LcWaikikiUrlPattern.IsMatch(url.Trim());
    }
    ...
    public async Task<ProductInspectResponse> InspectAsync(string url, CancellationToken cancellationToken = default)
    {
        if (!CanHandle(url))
        {
            throw new NotSupportedException($"LcWaikikiAdapter bu URL'yi desteklemiyor: {url}");
        }

        _logger.LogInformation("Inspecting LC Waikiki product: {Url}", url);
        ...
        try
        {
            ...
            page.Response += async (_, response) =>
            {
                try
                {
                    ...
                    if ((rUrl.Contains("/api/product") || rUrl.Contains("/model/") || rUrl.Contains("/option/")) &&
            ...
            if (gotoResponse.Status is 403 or 429)
                throw new InvalidOperationException($"LC Waikiki sayfası erişimi reddetti (HTTP {gotoResponse.Status} - Bot Koruması/Rate Limit).");
            ...
            {
                throw new InvalidOperationException("LC Waikiki sayfası bot koruması nedeniyle erişimi engelledi.");
            }

            try
            {
                await page.WaitForSelectorAsync(
                    "script[type='application/ld+json'], button[data-size], .option-size, .size-area, h1",
            ...
            {
                _logger.LogInformation("LC Waikiki: Parsed via JSON-LD: {Name} ({Count} variants)", jsonLdResult.Name, jsonLdResult.Variants.Count);
                return jsonLdResult;
            }
            ...
            if (interceptedJson is not null)
            {
                ...
                {
                    _logger.LogInformation("LC Waikiki: Parsed via Intercepted API: {Name} ({Count} variants)", apiResult.Name, apiResult.Variants.Count);
                    return apiResult;
                }
            }
            ...
            {
                _logger.LogInformation("LC Waikiki: Parsed via Embedded State: {Name} ({Count} variants)", stateResult.Name, stateResult.Variants.Count);
                return stateResult;
            }
            ...
            {
                _logger.LogInformation("LC Waikiki: Parsed via DOM: {Name} ({Count} variants)", domResult.Name, domResult.Variants.Count);
                return domResult;
            }
    ...
    private ProductInspectResponse? ParseJsonLdElement(JsonElement root, string url)
    {
        ...
        var name = root.TryGetProperty("name", out var nameProp) ? nameProp.GetString() ?? "LC Waikiki Ürün" : "LC Waikiki Ürün";
    ...
    public ProductInspectResponse? TryParseInterceptedJson(string json, string url)
    {
        try
        {
            ...
            var name = root.TryGetProperty("name", out var np) ? np.GetString() ?? "LC Waikiki Ürün" : "LC Waikiki Ürün";
    ...
    private async Task<ProductInspectResponse?> TryParseDomAsync(IPage page, string url)
    {
        var name = await page.EvaluateAsync<string>(@"() => {
            const h1 = document.querySelector('h1.product-title, h1, .product-name, [data-testid=""product-name""]');
        ...
        var imageUrl = await page.EvaluateAsync<string?>(@"() => {
            const img = document.querySelector('.product-images img, .picture-box img, img.product-image');
        ...
        var variantsJson = await page.EvaluateAsync<string>(@"() => {
            ...
            const sizeButtons = document.querySelectorAll(
                'button[data-size], a[data-size], .option-size a, .size-area a, .size-area button, .product-size button, [aria-label*=""Beden""]'
            );
    ...
    {
        if (string.IsNullOrWhiteSpace(title)) return "LC Waikiki Ürün";
        var clean = Regex.Replace(title, @"\s*[-|]\s*LC Waikiki.*$", "", RegexOptions.IgnoreCase).Trim();
