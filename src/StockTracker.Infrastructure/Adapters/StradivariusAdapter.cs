...
/// <summary>
/// Universal store adapter for Stradivarius (stradivarius.com).
/// Part of the Inditex group.
/// </summary>
public class StradivariusAdapter : IStoreAdapter, IInspectableAdapter
{
    private readonly IBrowserService _browserService;
    private readonly ILogger<StradivariusAdapter> _logger;

    private static readonly Regex StradivariusUrlPattern =
        new(@"https?://([a-zA-Z0-9_.-]+\.)?stradivarius\.com(/|$|\?)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);

    public string StoreName => "Stradivarius";
    public string AdapterKey => "stradivarius";
    public IReadOnlyList<string> SupportedDomains { get; } = new[] { "stradivarius.com" };

    public StradivariusAdapter(IBrowserService browserService, ILogger<StradivariusAdapter> logger)
    {
    ...
    public bool CanHandle(string url)
    {
        ...
        return StradivariusUrlPattern.IsMatch(url.Trim());
    }
    ...
    public async Task<ProductInspectResponse> InspectAsync(string url, CancellationToken cancellationToken = default)
    {
        if (!CanHandle(url))
        {
            throw new NotSupportedException($"StradivariusAdapter bu URL'yi desteklemiyor: {url}");
        }

        _logger.LogInformation("Inspecting Stradivarius product: {Url}", url);
        ...
        try
        {
            ...
            try
            {
                await page.WaitForSelectorAsync(
                    "script[type='application/ld+json'], button[data-qa-anchor*='size'], button.size-item, .size-selector, h1",
            ...
            {
                _logger.LogInformation("Stradivarius: Parsed via JSON-LD: {Name} ({Count} variants)", jsonLdResult.Name, jsonLdResult.Variants.Count);
                return jsonLdResult;
            }
            ...
            if (interceptedJson is not null)
            {
                ...
                {
                    _logger.LogInformation("Stradivarius: Parsed via Intercepted API: {Name} ({Count} variants)", apiResult.Name, apiResult.Variants.Count);
                    return apiResult;
                }
            }
            ...
            {
                _logger.LogInformation("Stradivarius: Parsed via Embedded State: {Name} ({Count} variants)", stateResult.Name, stateResult.Variants.Count);
                return stateResult;
            }
            ...
            {
                _logger.LogInformation("Stradivarius: Parsed via DOM: {Name} ({Count} variants)", domResult.Name, domResult.Variants.Count);
                return domResult;
            }
    ...
    private ProductInspectResponse? ParseJsonLdElement(JsonElement root, string url)
    {
        ...
        var name = root.TryGetProperty("name", out var nameProp) ? nameProp.GetString() ?? "Stradivarius Ürün" : "Stradivarius Ürün";
    ...
    public ProductInspectResponse? TryParseInterceptedJson(string json, string url)
    {
        try
        {
            ...
            var name = productElem.TryGetProperty("name", out var np) ? np.GetString() ?? "Stradivarius Ürün" : "Stradivarius Ürün";
    ...
    private async Task<ProductInspectResponse?> TryParseDomAsync(IPage page, string url)
    {
        var name = await page.EvaluateAsync<string>(@"() => {
            const h1 = document.querySelector('h1.product-title, h1.product-name, h1, [data-qa-anchor=""product-detail-name""]');
        ...
        var imageUrl = await page.EvaluateAsync<string?>(@"() => {
            const img = document.querySelector('.image-item img, .product-images img, img.image-zoom, [data-qa-anchor=""product-detail-image""] img');
        ...
        var variantsJson = await page.EvaluateAsync<string>(@"() => {
            ...
            const sizeButtons = document.querySelectorAll(
                'button[data-qa-anchor*=""size""], button.size-list-item, .sizes-list button, .ui-size-selector button, button.size-item, [aria-label*=""Size""], [aria-label*=""Beden""]'
            );
    ...
    {
        if (string.IsNullOrWhiteSpace(title)) return "Stradivarius Ürün";
        var clean = Regex.Replace(title, @"\s*[-|]\s*Stradivarius.*$", "", RegexOptions.IgnoreCase).Trim();
