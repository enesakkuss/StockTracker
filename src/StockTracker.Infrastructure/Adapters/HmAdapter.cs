...
/// <summary>
/// Universal store adapter for H&M (hm.com).
/// </summary>
public class HmAdapter : IStoreAdapter, IInspectableAdapter
{
    private readonly IBrowserService _browserService;
    private readonly ILogger<HmAdapter> _logger;

    private static readonly Regex HmUrlPattern =
        new(@"https?://([a-zA-Z0-9_.-]+\.)?hm\.com(/|$|\?)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);

    public string StoreName => "H&M";
    public string AdapterKey => "hm";
    public IReadOnlyList<string> SupportedDomains { get; } = new[] { "hm.com" };

    public HmAdapter(IBrowserService browserService, ILogger<HmAdapter> logger)
    {
    ...
    public bool CanHandle(string url)
    {
        ...
        return HmUrlPattern.IsMatch(url.Trim());
    }
    ...
    public async Task<ProductInspectResponse> InspectAsync(string url, CancellationToken cancellationToken = default)
    {
        if (!CanHandle(url))
        {
            throw new NotSupportedException($"HmAdapter bu URL'yi desteklemiyor: {url}");
        }

        _logger.LogInformation("Inspecting H&M product: {Url}", url);
        ...
        try
        {
            ...
            page.Response += async (_, response) =>
            {
                try
                {
                    ...
                    if ((rUrl.Contains("/api/product") || rUrl.Contains("/products/") || rUrl.Contains("/hm/v1/")) &&
                        response.Headers.TryGetValue("content-type", out var ct) && ct.Contains("json"))
                    {
                        var text = await response.TextAsync();
                        if (text.Contains("\"sizes\"") || text.Contains("\"variants\"") || text.Contains("\"inStock\""))
            ...
            if (gotoResponse.Status is 403 or 429)
                throw new InvalidOperationException($"H&M sayfası erişimi reddetti (HTTP {gotoResponse.Status} - Bot Koruması/Rate Limit).");
            ...
            {
                throw new InvalidOperationException("H&M sayfası bot koruması nedeniyle erişimi engelledi.");
            }

            try
            {
                await page.WaitForSelectorAsync(
                    "script[type='application/ld+json'], button[data-testid*='size'], .size-selector, .product-size, h1",
            ...
            {
                _logger.LogInformation("H&M: Parsed via JSON-LD: {Name} ({Count} variants)", jsonLdResult.Name, jsonLdResult.Variants.Count);
                return jsonLdResult;
            }
            ...
            if (interceptedJson is not null)
            {
                ...
                {
                    _logger.LogInformation("H&M: Parsed via Intercepted API: {Name} ({Count} variants)", apiResult.Name, apiResult.Variants.Count);
                    return apiResult;
                }
            }
            ...
            {
                _logger.LogInformation("H&M: Parsed via Embedded State: {Name} ({Count} variants)", stateResult.Name, stateResult.Variants.Count);
                return stateResult;
            }
            ...
            {
                _logger.LogInformation("H&M: Parsed via DOM: {Name} ({Count} variants)", domResult.Name, domResult.Variants.Count);
                return domResult;
            }
    ...
    private ProductInspectResponse? ParseJsonLdElement(JsonElement root, string url)
    {
        ...
        var name = root.TryGetProperty("name", out var nameProp) ? nameProp.GetString() ?? "H&M Ürün" : "H&M Ürün";
    ...
    public ProductInspectResponse? TryParseInterceptedJson(string json, string url)
    {
        try
        {
            ...
            var name = root.TryGetProperty("name", out var np) ? np.GetString() ?? "H&M Ürün" : "H&M Ürün";
            ...
            if (root.TryGetProperty("variants", out var varsElem) && varsElem.ValueKind == JsonValueKind.Array)
            {
                ...
                {
                    var sName = v.TryGetProperty("sizeFilter", out var sfp) ? sfp.GetString()
                              : v.TryGetProperty("size", out var sp) ? sp.GetString()
    ...
    private async Task<ProductInspectResponse?> TryParseDomAsync(IPage page, string url)
    {
        var name = await page.EvaluateAsync<string>(@"() => {
            const h1 = document.querySelector('h1.product-item-headline, h1, [data-testid=""product-name""]');
        ...
        var imageUrl = await page.EvaluateAsync<string?>(@"() => {
            const img = document.querySelector('.product-detail-main-image-container img, .product-image img, img.product-detail-thumbnail-image');
        ...
        var variantsJson = await page.EvaluateAsync<string>(@"() => {
            ...
            const sizeButtons = document.querySelectorAll(
                'button[data-testid*=""size""], .size-selector button, .picker-list button, ul.sizes button, [aria-label*=""Size""], [aria-label*=""Beden""]'
            );
    ...
    {
        if (string.IsNullOrWhiteSpace(title)) return "H&M Ürün";
        var clean = Regex.Replace(title, @"\s*[-|]\s*H&M.*$", "", RegexOptions.IgnoreCase).Trim();
